...
def createTabAtCircleEdgeSketch(
    ...
    circleSketch, circle = createCircleAtPointSketch(
        plane, radius, circleCenterPoint, targetComponent
    )
    circleCenterOnSketch = geometryUtils.pointToXY(
        circleSketch.modelToSketchSpace(circleCenterPoint)
    )
    angularPointOnSketch = geometryUtils.pointToXY(
        circleSketch.modelToSketchSpace(
    ...
    verticalConstructionLine = circleSketch.sketchCurves.sketchLines.addByTwoPoints(
        ...
        adsk.core.Point3D.create(
            circleCenterOnSketch.x,
            circleCenterOnSketch.y + radius,
            circleCenterOnSketch.z,
        ),
    )
    ...
    diagonalConstructionLine = circleSketch.sketchCurves.sketchLines.addByTwoPoints(
        circleCenterOnSketch, angularPointOnSketch
    )
    ...
    dimensions.addAngularDimension(
        diagonalConstructionLine, verticalConstructionLine, angularPointOnSketch
    )
    constraints.addCoincident(
        verticalConstructionLine.startSketchPoint, circle.centerSketchPoint
    )
    constraints.addCoincident(verticalConstructionLine.endSketchPoint, circle)
    constraints.addCoincident(
        diagonalConstructionLine.startSketchPoint, circle.centerSketchPoint
    )
    constraints.addCoincident(diagonalConstructionLine.endSketchPoint, circle)
    circle = circleSketch.sketchCurves.sketchCircles.addByCenterRadius(
        diagonalConstructionLine.endSketchPoint, radius / 2
    )
    dimensions.addRadialDimension(circle, circleCenterOnSketch)

    return circleSketch


def createSingleGridfinityBaseBody(
    input: BaseGeneratorInput, targetComponent: adsk.fusion.Component
):
    ...
    baseConstructionPlaneInput: adsk.fusion.ConstructionPlaneInput = (
        targetComponent.constructionPlanes.createInput()
    )
    baseConstructionPlaneInput.setByOffset(
        targetComponent.xYConstructionPlane,
        adsk.core.ValueInput.createByReal(input.originPoint.z),
    )
    baseConstructionPlane = targetComponent.constructionPlanes.add(
        baseConstructionPlaneInput
    )
    ...
    createRectangle(
        actual_base_width,
        actual_base_length,
        basePlateSketch.modelToSketchSpace(input.originPoint),
        basePlateSketch,
    )

    # extrude top section
    topSectionExtrudeDepth = adsk.core.ValueInput.createByReal(
        const.BIN_BASE_TOP_SECTION_HEIGH
    )
    topSectionExtrudeInput = extrudeFeatures.createInput(
        basePlateSketch.profiles.item(0),
        adsk.fusion.FeatureOperations.NewBodyFeatureOperation,
    )
    topSectionExtrudeExtent = adsk.fusion.DistanceExtentDefinition.create(
        topSectionExtrudeDepth
    )
    topSectionExtrudeInput.setOneSideExtent(
        topSectionExtrudeExtent,
    ...
    fillet_edges = edgeUtils.selectEdgesByLength(
        baseBody.faces, const.BIN_BASE_TOP_SECTION_HEIGH, const.DEFAULT_FILTER_TOLERANCE
    )
    filletInput.edgeSetInputs.addConstantRadiusEdgeSet(
        fillet_edges, adsk.core.ValueInput.createByReal(input.cornerFilletRadius), True
    )
    ...
    chamferInput.chamferEdgeSets.addEqualDistanceChamferEdgeSet(
        chamfer_edges, topSectionExtrudeDepth, True
    )
    ...
    if input.hasBottomChamfer:
        ...
        chamferFeatures: adsk.fusion.ChamferFeatures = features.chamferFeatures
        chamferInput = chamferFeatures.createInput2()
        chamfer_edges = commonUtils.objectCollectionFromList(
            faceUtils.getBottomFace(baseBottomExtrude.bodies.item(0)).edges
        )
        chamferInput.chamferEdgeSets.addEqualDistanceChamferEdgeSet(
            chamfer_edges,
    ...
    cutoutCenterPoint = adsk.core.Point3D.create(
        baseHoleCenterPoint.x, baseHoleCenterPoint.y, 0
    )
    ...
    if input.hasMagnetCutouts:
        ...
        if (
            input.hasScrewHoles
            and (const.BIN_BASE_HEIGHT - input.magnetCutoutsDepth)
            ...
            grooveLayer1 = shapeUtils.simpleBox(
                ...
                adsk.core.Point3D.create(
                    baseHoleCenterPoint.x + input.magnetCutoutsDiameter / 2,
                    baseHoleCenterPoint.y - input.screwHolesDiameter / 2,
                    0,
                ),
                targetComponent,
            )
            ...
            grooveLayer2 = shapeUtils.simpleBox(
                ...
                -const.BIN_MAGNET_HOLE_GROOVE_DEPTH / 2,
                adsk.core.Point3D.create(
                    baseHoleCenterPoint.x + input.screwHolesDiameter / 2,
                    baseHoleCenterPoint.y - input.screwHolesDiameter / 2,
                    0,
                ),
                targetComponent,
            ...
            combineUtils.intersectBody(
                grooveBody,
                commonUtils.objectCollectionFromList([grooveLayer1, grooveLayer2]),
                targetComponent,
            )

            rotateGrooveBodyInput = targetComponent.features.moveFeatures.createInput2(
                commonUtils.objectCollectionFromList([grooveBody])
            )
            verticalAxisVector = adsk.core.Vector3D.create(0, 0, 1.0)
            transformRotate = adsk.core.Matrix3D.create()
            transformRotate.setToRotation(
                math.radians(-45), verticalAxisVector, cutoutCenterPoint
            )
            rotateGrooveBodyInput.defineAsFreeMove(transformRotate)
            ratateGrooveBodyFeature = targetComponent.features.moveFeatures.add(
                rotateGrooveBodyInput
            )
    ...
    if input.hasScrewHoles or input.hasMagnetCutouts:
        if cutoutBodies.count > 1:
            joinFeature = combineUtils.joinBodies(
                cutoutBodies.item(0),
                commonUtils.objectCollectionFromList(list(cutoutBodies)[1:]),
                targetComponent,
            )
        ...
        baseXZMidPlaneInput.setByOffset(
            targetComponent.xZConstructionPlane,
            adsk.core.ValueInput.createByReal(input.baseLength / 2 - input.xyClearance),
        )
        ...
        baseYZMidPlaneInput.setByOffset(
            targetComponent.yZConstructionPlane,
            adsk.core.ValueInput.createByReal(input.baseWidth / 2 - input.xyClearance),
        )
        ...
        patternAxisInput.setByTwoPlanes(baseXZMidPlane, baseYZMidPlane)
        ...
        patternInput = circularPatternFeatures.createInput(cutoutBodies, baseCenterAxis)
        ...
        combineUtils.cutBody(
            ...
            commonUtils.objectCollectionFromList(
                list(cutoutBodies) + list(patternFeature.bodies)
            ),
            targetComponent,
        )
...
def createSingleBaseBodyWithClearance(
    input: BaseGeneratorInput, targetComponent: adsk.fusion.Component
):
    """
    ...
    offsetFacesInput = features.offsetFeatures.createInput(
        commonUtils.objectCollectionFromList(
            [face for face in list(baseBody.faces) if not faceUtils.isZNormal(face)]
        ),
        adsk.core.ValueInput.createByReal(0),
        adsk.fusion.FeatureOperations.NewBodyFeatureOperation,
        False,
    ...
    offsetSurfaceBodies = [
        body for body in list(offsetFacesFeature.bodies) if not body.isSolid
    ][0]
    ...
    thickenFaces = commonUtils.objectCollectionFromList(
        offsetFacesFeature.faces, extendFeature.faces
    )
    ...
    clearanceBoundingBox = extrudeUtils.createBoxAtPoint(
        ...
        targetComponent,
        adsk.core.Point3D.create(
            input.originPoint.x - input.xyClearance,
    ...
    combineUtils.joinBodies(
        baseBody,
        commonUtils.objectCollectionFromList(thickenFeaure.bodies),
        targetComponent,
    )
    return baseBody
...
def createBaseBodyPattern(
    ...
    patternInput.distanceTwo = adsk.core.ValueInput.createByReal(
        baseConfiguration.baseLength
    )
...
def cutBaseClearance(
    ...
    actual_base_width = (
        baseConfiguration.baseWidth * basesXCount - baseConfiguration.xyClearance * 2
    )
    actual_base_length = (
        baseConfiguration.baseLength * basesYCount - baseConfiguration.xyClearance * 2
    )
    features = targetComponent.features
    baseConstructionPlaneInput: adsk.fusion.ConstructionPlaneInput = (
        targetComponent.constructionPlanes.createInput()
    ...
    baseConstructionPlaneInput.setByOffset(
        targetComponent.xYConstructionPlane,
        adsk.core.ValueInput.createByReal(baseConfiguration.originPoint.z),
    )
    baseConstructionPlane = targetComponent.constructionPlanes.add(
        baseConstructionPlaneInput
    )
    ...
    fillet1 = sketchArcs.addFillet(
        side1,
        side1.endSketchPoint.geometry,
        side2,
        side2.startSketchPoint.geometry,
        filletRadius,
    )
    fillet2 = sketchArcs.addFillet(
        side2,
        side2.endSketchPoint.geometry,
        side3,
        side3.startSketchPoint.geometry,
        filletRadius,
    )
    fillet3 = sketchArcs.addFillet(
        side3,
        side3.endSketchPoint.geometry,
        side4,
        side4.startSketchPoint.geometry,
        filletRadius,
    )
    fillet4 = sketchArcs.addFillet(
        side4,
        side4.endSketchPoint.geometry,
        side1,
        side1.startSketchPoint.geometry,
        filletRadius,
    )
    ...
    baseClearanceCutSketch.offset(
        commonUtils.objectCollectionFromList(
            [fillet1, fillet2, fillet3, fillet4, side1, side2, side3, side4]
        ),
    ...
    cuttingProfile = min(
        list(baseClearanceCutSketch.profiles), key=lambda x: x.boundingBox.minPoint.x
    )
    clearanceCutExtrudeInput = features.extrudeFeatures.createInput(
        cuttingProfile, adsk.fusion.FeatureOperations.CutFeatureOperation
    )
